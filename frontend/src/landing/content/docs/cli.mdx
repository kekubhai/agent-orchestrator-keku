...
References become stale after navigation, DOM replacement, frame changes, or tab changes. Take another snapshot when AO reports `STALE_REFERENCE`. `ao browser snapshot --delta` returns the full tree once, then only `unchanged` or the changes since the last snapshot the agent received; add `--full` to get the full tree again. `click` and `drag` accept `--human` for curved pointer movement on pages that ignore instant clicks, and `ao browser screenshot --annotate` numbers interactive elements in the image and lists their refs. `--human` actions and screenshots need the Browser panel visible on screen; a hidden or covered window stops painting and those commands time out. Network capture is off by default, metadata-only, bounded, and automatically expires.
